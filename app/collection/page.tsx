        ...
        {collection.length > 0 && (
          ...
          >
            {RARITY_ORDER.map((rarity) => (
              <div
                key={rarity}
                className={`rounded-2xl border p-4 ${RARITY_STYLE[rarity].card} bg-white shadow-sm`}
              >
          ...
          <div className="mt-8 flex flex-wrap gap-2">
            {filterOptions.map((opt) => (
              ...
              >
                {opt.label}
                {opt.count > 0 && (
                  <span className={`ml-1.5 ${filter === opt.id ? "text-white/60" : "text-gray-400"}`}>
                    {opt.count}
                  </span>
                )}
              </button>
        ...
        <div className="mt-6 flex-1">
          {collection.length === 0 ? (
            ...
            >
              ...
              <a
                href="/"
                className="mt-6 rounded-full bg-ink px-6 py-3 text-sm font-black text-white transition-colors hover:bg-gray-800"
              >
                Start swiping
              </a>
            </motion.div>
            ...
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              {filtered.map((item, i) => (
                <motion.a
                  ...
                  className={`group rounded-2xl border bg-white p-4 shadow-sm transition-all hover:-translate-y-0.5 hover:shadow-card-lg ${RARITY_STYLE[item.rarity].card}`}
                >
